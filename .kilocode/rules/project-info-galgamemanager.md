...
*   **`Models/BgTasks/PvnSyncTasks/`**: Contains specialized background tasks for PotatoVN synchronization:
    ...
    *   Pending PotatoVN game/staff deletion queues are stored through `PvnPendingDeletionStore` using large settings. It migrates and deduplicates the legacy application-settings values so large queues cannot exceed the packaged Windows settings-container size limit.
